...
fn diff(
    ...
    let node = store
        .get(id)
        .cloned()
        .ok_or(CalculusError::MissingExpression(id.0))?;
    ...
    let result = match node {
        ...
        ExprNode::Div {
            ...
            let bottom = store.intern(ExprNode::PowI {
                base: denominator,
                exponent: 2,
            });
        ...
        ExprNode::PowI { base, exponent } => {
            if exponent == 0 {
                ...
                let power = store.intern(ExprNode::PowI {
                    base,
                    exponent: exponent - 1,
                });
        ...
        ExprNode::Apply { function, args } => {
            if args.len() != 1 {
                ...
                let outer = match function.as_str() {
                    ...
                    _ => store.intern(ExprNode::Derivative {
                        expr: id,
                        with_respect_to: wrt,
                        order: 1,
                    }),
...
fn subst(
    ...
    let out = match node {
        ...
        ExprNode::Neg(x) => {
            let x = subst(store, x, replacements, memo)?;
            store.intern(ExprNode::Neg(x))
        }
        ExprNode::Add(xs) => {
            let xs = xs
                .into_iter()
                .map(|x| subst(store, x, replacements, memo))
                .collect::<Result<Vec<_>, _>>()?;
            store.add(xs)
        }
        ExprNode::Mul(xs) => {
        ...
        ExprNode::Div {
            ...
            let n = subst(store, numerator, replacements, memo)?;
            let d = subst(store, denominator, replacements, memo)?;
            store.intern(ExprNode::Div {
                numerator: n,
                denominator: d,
            })
        ...
        ExprNode::PowI { base, exponent } => {
            let base = subst(store, base, replacements, memo)?;
        ...
        ExprNode::Derivative {
            ...
            let expr = subst(store, expr, replacements, memo)?;
            store.intern(ExprNode::Derivative {
                expr,
                with_respect_to,
                order,
...
fn eval(
    ...
    let value = match node {
        ExprNode::Literal(ScalarLiteral::Integer(v)) => v
            .parse()
            .map_err(|_| CalculusError::Evaluation(format!("bad integer literal {v}")))?,
        ExprNode::Literal(ScalarLiteral::Rational {
            numerator,
            denominator,
        }) => {
            let n: f64 = numerator
                .parse()
                .map_err(|_| CalculusError::Evaluation("bad rational numerator".into()))?;
            let d: f64 = denominator
                .parse()
                .map_err(|_| CalculusError::Evaluation("bad rational denominator".into()))?;
            n / d
        }
        ExprNode::Literal(ScalarLiteral::FloatBits(bits)) => f64::from_bits(*bits),
        ExprNode::Literal(ScalarLiteral::NamedConstant(name)) if name == "pi" => {
            std::f64::consts::PI
        }
        ExprNode::Literal(ScalarLiteral::NamedConstant(name)) if name == "e" => std::f64::consts::E,
        ExprNode::Literal(ScalarLiteral::NamedConstant(name)) => {
            return Err(CalculusError::Evaluation(format!(
                "unknown named constant {name}"
            )));
        }
        ExprNode::Symbol(symbol) => *values.get(symbol).ok_or_else(|| {
            CalculusError::Evaluation(format!("missing value for symbol {symbol}"))
        })?,
        ExprNode::Neg(x) => -eval(store, *x, values, memo)?,
        ExprNode::Add(xs) => xs
            .iter()
            .try_fold(0.0, |acc, x| Ok(acc + eval(store, *x, values, memo)?))?,
        ExprNode::Mul(xs) => xs
            .iter()
            .try_fold(1.0, |acc, x| Ok(acc * eval(store, *x, values, memo)?))?,
        ExprNode::Div {
            numerator,
            denominator,
        } => eval(store, *numerator, values, memo)? / eval(store, *denominator, values, memo)?,
        ...
        ExprNode::Derivative { .. } => {
            return Err(CalculusError::Evaluation(
                "semantic derivative must be lowered before numeric evaluation".into(),
            ));
        }
    };
