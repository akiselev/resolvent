...
use crate::latex::{MathEquation, MathExpr};
use crate::units::{Dimension, UnitExpr};
use std::collections::BTreeMap;

pub fn check_model(model: &ParsedModel) -> Vec<Diagnostic> {
    let mut dims = BTreeMap::<String, Dimension>::new();
    let mut functions = BTreeMap::<String, Dimension>::new();
    for decl in &model.declarations {
        if let Some(UnitExpr { dimension, .. }) = &decl.unit {
            match decl.kind {
                DeclKind::Coefficient | DeclKind::Source => {
                    functions.insert(decl.name.clone(), *dimension);
                }
                _ => {
                    dims.insert(decl.name.clone(), *dimension);
                }
            }
        }
    }
    ...
    dims.insert("y".into(), Dimension::LENGTH);
...
fn check_equation(
    ...
    functions: &BTreeMap<String, Dimension>,
    out: &mut Vec<Diagnostic>,
) {
    match (
        infer(&eq.lhs, dims, functions),
        infer(&eq.rhs, dims, functions),
    ) {
        (Ok(a), Ok(b)) if a != b => out.push(
            Diagnostic::error(
                "RSL-U014",
                "units",
                format!("equation dimensions disagree: left is `{a}`, right is `{b}`"),
            )
            .at(span, "dimension mismatch in this equation"),
        ),
        (Err(m), _) | (_, Err(m)) => out.push(
            Diagnostic::error("RSL-U013", "units", m).at(span, "while checking this equation"),
        ),
        _ => {}
    }
}
...
fn infer(
    ...
    Ok(match expr {
        ...
        MathExpr::Name(name) => dims
            .get(name)
            .copied()
            .or_else(|| functions.get(name).copied())
        ...
        MathExpr::Add(xs) => {
            ...
            let first = it
                .next()
                .map(|x| infer(x, dims, functions))
                .transpose()?
                .unwrap_or(Dimension::DIMENSIONLESS);
            for x in it {
                let d = infer(x, dims, functions)?;
                if d != first {
                    return Err(format!(
                        "additive terms have dimensions `{first}` and `{d}`"
                    ));
                }
            }
            first
        ...
        MathExpr::Pow { base, exponent } => infer(base, dims, functions)?.powi(
            (*exponent)
                .try_into()
                .map_err(|_| "unit exponent outside i8 range".to_string())?,
        ),
