...
pub fn analyze_dae(
    system: &System,
    exprs: &ExprStore,
) -> Result<StructuralDaeAnalysis, StructuralError> {
    ...
    let estimated_structural_index = pantelides
        .iter()
        .map(|s| s.differentiate_times)
        .max()
        .unwrap_or(0)
        .saturating_add(if derivative_variables.is_empty() {
            0
        } else {
            1
        });
    let dummy_derivatives = select_dummy_derivatives(&derivative_variables, &pantelides);
    Ok(StructuralDaeAnalysis {
        incidence,
        matching,
        ...
        pantelides,
        dummy_derivatives,
        estimated_structural_index,
    })
}

pub fn collect_derivative_variables(
    system: &System,
    exprs: &ExprStore,
) -> Result<Vec<DerivativeVariable>, StructuralError> {
    ...
    while let Some(id) = stack.pop() {
        ...
        match node {
            ExprNode::Derivative {
                ...
                if let Some(ExprNode::Symbol(base)) = exprs.get(*expr) {
...
fn direct_symbol(id: ExprId, exprs: &ExprStore) -> Result<Option<SymbolId>, StructuralError> {
    Ok(
        match exprs
            .get(id)
            .ok_or(StructuralError::MissingExpression(id.0))?
...
pub fn pantelides_plan(
    system: &System,
    exprs: &ExprStore,
    incidence: &IncidenceSystem,
    matching: &Matching,
    derivatives: &[DerivativeVariable],
) -> Result<Vec<PantelidesStep>, StructuralError> {
    if matching.is_perfect() || derivatives.is_empty() {
        return Ok(Vec::new());
    }
    let mut derivative_bases = BTreeSet::new();
    for derivative in derivatives {
        derivative_bases.insert(derivative.base);
    }
    let variable_columns: BTreeMap<SymbolId, usize> = incidence
        .variables
        .iter()
        .copied()
        .enumerate()
    ...
    for (equation_index, row) in incidence.rows.iter().enumerate() {
        let touches_unmatched_differential = row.iter().any(|column| {
            unmatched.contains(column) && derivative_bases.contains(&incidence.variables[*column])
        });
    ...
    if steps.is_empty() && !matching.unmatched_equations().is_empty() {
        for equation in matching.unmatched_equations() {
            let touches_differential = incidence.rows[equation]
                .iter()
                .any(|column| derivative_bases.contains(&incidence.variables[*column]));
            if touches_differential {
                steps.push(PantelidesStep {
                    equation,
                    differentiate_times: 1,
                    reason: "unmatched equation is incident to a differential variable".into(),
                });
            }
        }
    }
...
pub fn select_dummy_derivatives(
    derivatives: &[DerivativeVariable],
    pantelides: &[PantelidesStep],
) -> Vec<DummyDerivative> {
    if pantelides.is_empty() {
        return Vec::new();
    }
...
mod tests {
    ...
    fn finds_aliases_and_derivatives() {
        ...
        let x = symbols.declare(Symbol {
            name: "x".into(),
            role: SymbolRole::State,
            dimension: None,
        });
        let y = symbols.declare(Symbol {
            name: "y".into(),
            role: SymbolRole::Algebraic,
            dimension: None,
        });
        let t = symbols.declare(Symbol {
            name: "t".into(),
            role: SymbolRole::Independent,
            dimension: None,
        });
        let mut exprs = ExprStore::new();
        let ex = exprs.symbol(x);
        let ey = exprs.symbol(y);
        let dx = exprs.intern(ExprNode::Derivative {
            expr: ex,
            with_respect_to: t,
            order: 1,
        });
        ...
        assert_eq!(
            collect_derivative_variables(&system, &exprs).unwrap().len(),
            1
        );
    }
}
