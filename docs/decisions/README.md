...
| [017](ADR-017-layer-4-egraph-seam.md) | L4 is a resolvent-owned hash-consed DAG with a caller-owned `FuncTable`; no e-graph dependency; one explicit value-preserving `canonicalize`. ~~No `simplify`, no `RuleSet`, no rewriter~~ **§2 bullet 3 and §5–§6 superseded 2026-08-08 — see the table in its header before reading the body** | cheap | Ratified, **superseded in part by 032/033** |
