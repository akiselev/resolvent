...
fn run() -> Result<(), String> {
    ...
    let input = args.next().ok_or("usage: update-sirp <sirp.json> <snapshot.json>")?;
    let output = args.next().ok_or("usage: update-sirp <sirp.json> <snapshot.json>")?;
    let bytes = fs::read(&input).map_err(|e| format!("{input}: {e}"))?;
    let value: serde_json::Value = serde_json::from_slice(&bytes).map_err(|e| format!("{input}: {e}"))?;
...
fn canonicalize(value: serde_json::Value) -> serde_json::Value {
    match value {
        serde_json::Value::Array(xs) => serde_json::Value::Array(xs.into_iter().map(canonicalize).collect()),
        serde_json::Value::Object(map) => {
            let mut entries: Vec<_> = map.into_iter().collect();
            entries.sort_by(|a, b| a.0.cmp(&b.0));
            let mut out = serde_json::Map::new();
            for (k, v) in entries { out.insert(k, canonicalize(v)); }
